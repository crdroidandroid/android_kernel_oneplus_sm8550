					...
					u8 allowed_voltage)
{
	...
	if (chg->chg_param.smb_version == PM660) {
...
/* FG GEN3 channels */
static const char * const smblib_fg_gen3_ext_iio_chan[] = {
	[SMB2_FG_GEN3_DEBUG_BATTERY] = "debug_battery",
	[SMB2_FG_GEN3_CAPACITY] = "capacity",
	[SMB2_FG_GEN3_VOLTAGE_NOW] = "voltage_now",
	[SMB2_FG_GEN3_CURRENT_NOW] = "current_now",
	[SMB2_FG_GEN3_CHARGE_FULL] = "charge_full",
	[SMB2_FG_GEN3_CHARGE_COUNTER] = "charge_counter",
	[SMB2_FG_GEN3_CYCLE_COUNT] = "cycle_count",
	[SMB2_FG_GEN3_CHARGE_FULL_DESIGN] = "charge_full_design",
	[SMB2_FG_GEN3_TIME_TO_FULL_NOW] = "time_to_full_now",
	[SMB2_FG_GEN3_TEMP] = "temp",
};
...
void smblib_suspend_on_debug_battery(struct smb_charger *chg)
{
	...
	rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_DEBUG_BATTERY, &val);
			...
			void *data, int disable, const char *client)
{
	...
	if (chg->connector_type == QTI_POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		...
		pval.intval = disable ? QTI_POWER_SUPPLY_TYPEC_PR_SINK
				  ...
				  union power_supply_propval *val)
{
	...
	if (chg->iio_chan_list_fg_gen3)
		rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_CAPACITY, &val->intval);
	if (rc < 0)
				...
				union power_supply_propval *val)
{
	...
	if (stat & CHARGER_ERROR_STATUS_BAT_OV_BIT) {
		rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_VOLTAGE_NOW,
				&pval.intval);
		if (rc < 0) {
				  ...
				  int *val)
{
	...
	*val = (bool)(stat & QNOVO_PT_ENABLE_CMD_BIT);
	return 0;
}
				...
				int channel, int *val)
{
	...
	if (IS_ERR_OR_NULL(chg->iio_chan_list_fg_gen3))
		return -ENODEV;

	rc = iio_read_channel_processed(chg->iio_chan_list_fg_gen3[channel],
		val);
...
int smblib_get_prop_pd_allowed(struct smb_charger *chg,
			       int *val)
{
	*val = get_effective_result(chg->pd_allowed_votable);
	return 0;
}
						...
						int *val)
{
	...
	case QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3:
		...
		*val = MICRO_5V + HVDCP3_STEP_UV * pulses;
		break;
	case POWER_SUPPLY_TYPE_USB_PD:
...
int smblib_get_prop_die_health(struct smb_charger *chg, int *val)
{
	int rc;
	u8 stat;
				     ...
				     int val)
{
	...
	if (!get_effective_result(chg->disable_power_role_switch))
		return __smblib_set_prop_typec_power_role(chg, &pval);
				    ...
				    int val)
{
	...
	if (val < 0 || val >= TYPEC_SRC_RP_MAX_ELEMENTS)
		return -EINVAL;
			      ...
			      int val)
{
	...
	return __smblib_set_prop_pd_active(chg, val);
}
	...
	if (chg->wa_flags & OV_IRQ_WA_BIT) {
		...
		if (pval.intval > MICRO_10P3V) {
			...
			vote(chg->hvdcp_hw_inov_dis_votable, OV_VOTER, true,
				0);
			pval.intval = QTI_POWER_SUPPLY_DP_DM_FORCE_5V;
			smblib_dp_dm(chg, pval.intval);
			return;
		}
	}
}
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int val)
{
	int typec_mode;
	int rp_ua;
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (!chg->typec_legacy_valid)
		smblib_force_legacy_icl(chg, apsd_result->val);
...
static int typec_try_sink(struct smb_charger *chg)
{
	...
	typec_mode = smblib_get_prop_typec_mode(chg);
	if (typec_mode == QTI_POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER
		|| typec_mode == QTI_POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY)
		return ATTACHED_SRC;
...
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode)
{
	...
	if ((apsd->val != POWER_SUPPLY_TYPE_USB_DCP)
		&& (apsd->val != QTI_POWER_SUPPLY_TYPE_USB_FLOAT))
		return;
...
static void smblib_handle_typec_cc_state_change(struct smb_charger *chg)
{
	...
	if (!chg->typec_present && chg->typec_mode != QTI_POWER_SUPPLY_TYPEC_NONE) {
		chg->typec_present = true;
...
static void bms_update_work(struct work_struct *work)
{
	...
	struct iio_channel **fg_gen3_list;
	int rc;

	if (IS_ERR(chg->iio_chan_list_fg_gen3))
		return;

	if (!chg->iio_chan_list_fg_gen3) {
		fg_gen3_list = get_ext_channels(chg->dev,
			smblib_fg_gen3_ext_iio_chan,
			ARRAY_SIZE(smblib_fg_gen3_ext_iio_chan));
		if (IS_ERR(fg_gen3_list)) {
			rc = PTR_ERR(fg_gen3_list);
			if (rc != -EPROBE_DEFER) {
				dev_err(chg->dev, "Failed to get channels, %d\n",
					rc);
				chg->iio_chan_list_fg_gen3 = ERR_PTR(-EINVAL);
			}
			return;
		}
		chg->iio_chan_list_fg_gen3 = fg_gen3_list;
	}
...
static void smblib_legacy_detection_work(struct work_struct *work)
{
	...
	rp_high = chg->typec_mode == QTI_POWER_SUPPLY_TYPEC_SOURCE_HIGH;
...
int smblib_init(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		iio_list = get_ext_channels(chg->dev, smblib_fg_gen3_ext_iio_chan,
			ARRAY_SIZE(smblib_fg_gen3_ext_iio_chan));
		if (!IS_ERR(iio_list))
			chg->iio_chan_list_fg_gen3 = iio_list;
