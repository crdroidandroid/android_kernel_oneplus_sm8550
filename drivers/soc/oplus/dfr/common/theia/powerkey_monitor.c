...
ssize_t get_recovery_pwkey_stage(char *buf)
{
	if (stage_start != flow_index) {
		int last_index = (flow_index < 2) ? (FLOW_SIZE - 2) : (flow_index - 2);
		snprintf(buf, 64, (last_index * STAGE_BRIEF_SIZE + flow_buf));
		POWER_MONITOR_DEBUG_PRINTK("get_recovery_pwkey_stage buf:%s,flow_index:%d\n", buf, flow_index);
	} else {
		sprintf(buf, "");
	}
	...
	char __user *buf, size_t count, loff_t *off)
{
	...
	POWER_MONITOR_DEBUG_PRINTK("enter theia_powerkey_report_proc_read %zu  %lld", count, *off);
...
void record_stage(const char *buf)
{
	...
	spin_unlock_irqrestore(&record_stage_spinlock, flag);
}
...
static char recovery_panic_stages[][64] = {
	{ "POWERKEY_START_BL" },
	{ "POWERKEY_START_BR" },
};
...
void recovery_timer_func(struct timer_list *t)
{
	...
	int len;
	char stage[64] = {0};;
	get_recovery_pwkey_stage(stage);

	POWER_MONITOR_DEBUG_PRINTK("recovery_timer_func: doRecovery.\n");
	len = ARRAY_SIZE(recovery_panic_stages);

	for (num = 0; num < len; num++) {
		if (!strcmp(stage, recovery_panic_stages[num])) {
			POWER_MONITOR_DEBUG_PRINTK("recovery_timer_func: need kernel reboot to recovery, last stage:%s,", stage);
			theia_send_event(THEIA_RECOVER_EVENT_PWK_KERNEL_MONITOR, 0, 0, 0);
			return;
		}
	}

	POWER_MONITOR_DEBUG_PRINTK("recovery_timer_func: need system reboot to recovery, last stage:%s,", stage);
	theia_send_event(THEIA_RECOVER_EVENT_PWK_SYSTEM_MONITOR, 0, 0, 0);
}
...
int __init powerkey_monitor_init(void)
{
	...
	spin_lock_init(&record_stage_spinlock);
