...
static int get_cluster_num(struct cpufreq_policy *policy)
{
	...
	if (platform_soc_id == SM8350_SOC_ID) {
		...
		if(opp_number[cluster_num] != sm7450_cluster_pd[cluster_num]) {
			/* pr_err("opp_number: %d  cluster_pd: %d. \n", opp_number[cluster_num], sm7450_cluster_pd[cluster_num]); */
			return -1;
		}
	}
...
unsigned int update_power_effiency_lock(struct cpufreq_policy *policy, unsigned int freq, unsigned int loadadj_freq)
{
	...
	if ((policy == NULL) || (affect_mode == 0) || (freq <= 0)) {
		return freq;
	}
...
void frequence_opp_init(struct cpufreq_policy *policy)
{
	...
	if(policy == NULL) {
		pr_err("the policy was NULL, return. \n");
		return;
	}
	...
	pr_info("opp_number: %d  cluster_id: %d. \n", opp_number[cluster_id], cluster_id);
}
